...
describe("AAR062: subscribe operations must declare a consumer group (AsyncAPI 2.x)", () => {
  ...
  test("Should flag a subscribe operation defined under components and reached via $ref", async () => {
    const results = await run(failRefComponentsNoGroup);
    expect(results.length).toBe(1);
    results.forEach((r) => expect(r.code).toBe("asa:AAR062"));
  });

  test("Should flag a subscribe operation defined under components even when unreferenced", async () => {
